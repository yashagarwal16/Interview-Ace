...
class SklearnSGDFLModel(SklearnFLModel):
    ...
    def update_model(self, model_update):
        ...
        if isinstance(model_update, ModelUpdate):
            ...
            # Scikitlearn has added float32 support.  Try to retain types.
            coefType = self.model.coef_.dtype if hasattr(self.model,"coef_") else None
            interceptType = self.model.intercept_.dtype if hasattr(self.model,"intercept_") else None

            if isinstance(self.model, SGDClassifier):
                coef = np.array(weights, dtype=coefType)[:, :-1]
                intercept = np.array(weights, dtype=interceptType)[:, -1]
            elif isinstance(self.model, SGDRegressor):
                coef = np.array(weights, dtype=coefType)[:-1]
                intercept = np.array(weights, dtype=interceptType)[-1].reshape(
                    1,
                )
            else:
