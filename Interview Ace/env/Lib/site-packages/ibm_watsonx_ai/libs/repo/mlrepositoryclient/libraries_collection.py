...
from ibm_watsonx_ai.libs.repo.mlrepositoryartifact.libraries_artifact import LibrariesArtifact
...
from ibm_watsonx_ai.libs.repo.swagger_client.models import LibrariesDefinitionInput, LibrariesDefinitionInputPlatform, PatchOperationLibraries
...
class LibrariesCollection:
    ...
    def get(self, libraries_id):
        ...
        if(libraries_id.__contains__("/v4/libraries")):
            matched = re.search(r'.*/v4/libraries/([A-Za-z0-9\-]+)', libraries_id)
    ...
    def save(self, artifact, library = None):
        ...
        """
        logger.debug('Creating a new Libraries : {}'.format(artifact.name))
