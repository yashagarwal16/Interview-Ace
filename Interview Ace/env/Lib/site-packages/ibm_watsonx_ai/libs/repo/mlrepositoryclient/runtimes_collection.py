...
from ibm_watsonx_ai.libs.repo.mlrepositoryartifact.runtimes_artifact import RuntimesArtifact
...
from ibm_watsonx_ai.libs.repo.swagger_client.models import RuntimeSpecDefinitionInput, RuntimeSpecDefinitionInputCustomLibraries,\
...
class RuntimesCollection:
    ...
    def get(self, runtimes_id):
        ...
        if(runtimes_id.__contains__("/v4/runtimes")):
            matched = re.search(r'.*/v4/runtimes/([A-Za-z0-9\-]+)', runtimes_id)
    ...
    def save(self, artifact, runtimespec_path  = None):
        ...
        """
        logger.debug('Creating a new Runtimes : {}'.format(artifact.name))
